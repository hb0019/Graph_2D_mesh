...
class Trainer():
    ...
    def train(self, graph, train_dataset):
        tot_loss_array = []
        for t in range(self.epochs):
            ...
            print(f"epoch: {t}\t epoch_loss: {loss}\t total_loss: {sum(tot_loss_array)/len(tot_loss_array)}\t acc: {acc}")
            loss.backward()
            self.optimizer.step()
        return 
...
if __name__ == "__main__": 
    ...
    trainer.train(Graph['graph'], Graph['train_dataset'])
    trainer.save()
